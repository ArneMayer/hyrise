...
typedef struct __attribute__ ((__packed__)) qfblock {
	...
	uint8_t   slots[SLOTS_PER_BLOCK * BITS_PER_SLOT32 / 8];
#else
	uint8_t   slots[];
#endif
} qfblock;
...
static inline uint64_t get_slot(const QF *qf, uint64_t index)
{
	...
	assert(index < qf->xnslots);
	uint64_t *p = (uint64_t *)&get_block(qf, index / SLOTS_PER_BLOCK)->slots[(index % SLOTS_PER_BLOCK) * BITS_PER_SLOT32 / 8];
	return (uint64_t)(((*p) >> (((index % SLOTS_PER_BLOCK) * BITS_PER_SLOT32) % 8)) & BITMASK(BITS_PER_SLOT32));
}
...
static inline void set_slot(const QF *qf, uint64_t index, uint64_t value)
{
	...
	uint64_t *p = (uint64_t *)&get_block(qf, index / SLOTS_PER_BLOCK)->slots[(index % SLOTS_PER_BLOCK) * BITS_PER_SLOT32 / 8];
	uint64_t t = *p;
	uint64_t mask = BITMASK(BITS_PER_SLOT32);
	uint64_t v = value;
	int shift = ((index % SLOTS_PER_BLOCK) * BITS_PER_SLOT32) % 8;
	mask <<= shift;
	v <<= shift;
	t &= ~mask;
	t |= v;
