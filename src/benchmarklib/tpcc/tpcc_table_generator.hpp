...
class TpccTableGenerator : public opossum::AbstractBenchmarkTableGenerator {
 ...
 public:
  ...
  static TpccTableGeneratorFunctions tpcc_table_generator_functions(opossum::ChunkOffset chunk_size, const size_t warehouse_size);

  static std::shared_ptr<Table> generate_tpcc_table(const std::string& tablename);
  static std::shared_ptr<opossum::Table> generate_tpcc_table(const std::string& tablename, opossum::ChunkOffset chunk_size, const size_t warehouse_size, bool compress);	
